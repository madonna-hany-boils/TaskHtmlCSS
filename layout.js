...
const tableData = [
    {
        test: "الأصول الغير متداولة المحتفظ بها في البيع",
        course: "أصول المعاملات والبيع",
        category: "معاملات",
        status: "مكتمل",
        progress: 40,
        action: "ابدأ الآن"
...
const completedBadge = `<span class="status-badge status-completed">مكتمل</span>`;
const readyBadge = `<span class="status-badge status-ready">جاهز للبدء</span>`;
const seeYourAnswers = `<img src="../assets/icons/eye-icon.svg" class="action-icon" alt="شاهد الاجابات" />`;
const retakeExam = `<img src="../assets/icons/retake-exam-icon.svg" class="action-icon" alt="جرب مره اخري" />`;
...
function renderTable(data) {
    ...
    data.forEach(item => {
        ...
        if (item.status === 'مكتمل') {
            let isSuccessful = item.progress < 50 ? 'progress-failed' : item.progress >= 75 ? 'progress-exlent' : 'progress-good';

            statusBadge = completedBadge;
            statusIcons = `
                        <div class="status-icons">
                            <div class="status-icon retake-exam redirect-btn-to-exam" data-target="exam.html">
                                ${retakeExam}
                            </div>
                            <div class="status-icon see-your-answers redirect-btn-to-results" data-target="exam.result.html">
                                ${seeYourAnswers}
                            </div>
                        </div>
                    `;
            progressDisplay = `
                        <div class="d-flex align-items-center">
                            <span class="${isSuccessful} exam-percentage">${item.progress}%</span>
                        </div>
                    `;
        }
        else {
            ...
            statusIcons = `
                        <div class="status-icons">
                            <button class="action-btn" data-target="exam.html">ابدأ الآن</button>
                        </div>
                    `;
            progressDisplay = `
                        <div class="d-flex align-items-center">
...
document.addEventListener('click', function (e) {
    const button = e.target.closest('.redirect-btn-to-exam , .redirect-btn-to-results , .action-btn');
    if (button) {
